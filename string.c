...
string *deleteString( string *val )
{
    ...
    if ( ( val->used == 1 ) && ( val->mallocStart == val ) ) // it is the first string and the full block is useless
    {
        free( val->start );
        free( val );
    }
    ...
    {
        if ( val->mallocStart->used == 1 )
        { // it is not the first string but the full block is useless. Can
        ...
        {
            if ( val->mallocStart == val ) // it is the first string but the full block is useful
            {
                val->used--;
            }
...
string *spiltString( string *st, char aim )
{
    ...
    val->sizeIndex = st->sizeIndex; // It is meaningless. Just to init. The value
                                    // will not be updated.
    val->used = 1;                  // It is meaningless. Just to init. The value will not be updated.
